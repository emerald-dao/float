...
const claimFloat = async (eventId: string, eventCreator: string, claimCode: string | undefined) => {
	...
	if (claimCode) {
		...
		return await claimFLOATExecution(eventId, eventCreator, secretSig, actionAfterClaimFloat);
	}

	return await claimFLOATExecution(eventId, eventCreator, null, actionAfterClaimFloat);
};
