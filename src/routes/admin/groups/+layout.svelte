<script lang="ts">
	...
	let groups = createFetchStore<Group[]>(
		() => fetch(`/api/groups/${$user.addr}`).then((res) => res.json()),
		[]
	);
	...
	<div class="left-wrapper">
				...
				{:else}
					...
					>
						<div class="groups-wrapper">
							{#each $searchStore.filtered as group, i (group.group.id)}
		...
		<div class="pagination-wrapper row-space-between row-6">
			<Pagination
				...
				bind:paginationMin
			/>
			<Button href={`/admin/groups/new-group`} size="small">
