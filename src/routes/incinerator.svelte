<script>
  import IncineratorTable from "$lib/components/common/table/IncineratorTable.svelte";
  import { selectedToBurn } from "$lib/components/common/table/stores";
  ...
  const receiveEvent = (message) => {
    ...
    console.log(message);
    if (message.flowEventId === "A.2d4c3caffbeab845.FLOAT.FLOATDestroyed") {
      renderImage(
        message.id,
  ...
  function deleteMyFloats() {
    showTable = false;
    const ids = Object.keys($selectedToBurn);
    console.log("Ids to burn: " + ids);
    incinerate(ids);
  }
</script>

<svelte:head>
  <title>Incinerator {PAGE_TITLE_EXTENSION}</title>
</svelte:head>
...
{#if !showTable}
  <div class="spawner noTable">
    ...
    {:else if $incinerateInProgress}
      <button id="incinerate-button" aria-busy="true" disabled
        >Incinerating...</button>
    ...
    {:else}
      <button id="incinerate-button" on:click={() => (showTable = true)}
        >Incinerate FLOATs</button>
    {/if}
  ...
  {#each Object.keys(claimedEvents) as id (id)}
    <Lantern ipfsHash={claimedEvents[id].ipfsHash} direction={"incineration"} />
  {/each}
{:else}
  <div class="spawner">
    {#await floats then floats}
      <div id="table-wrapper">
        <article>
          <div id="burn-header">
            <h2>FLOATs to Burn</h2>
            <button id="burn-button" on:click={deleteMyFloats}>Burn!</button>
...
<style>
  ...
  #incinerate-button {
    ...
    width: auto;
    border: 0;
    background-color: #35e8c5;
    box-shadow: 4px 4px 25px #35e8c5;
    color: black;
  }
  ...
  .spawner {
    ...
    position: relative;
    width: 100vw;
    background: linear-gradient(#12191f, #006851);
  }
  .noTable {
    min-height: 100vh;
  }
