<script>
  ...
  export let goal;
  /** @type {number} */
  export let totalSlots;

  /**
   * @param rawGoal
   * @return {import('../types').EventSeriesAchievementGoal}
   */
  function parseRawGoalData(rawGoal) {
    /** @type {import('../types').GoalType} */
    let type;
    /** @type {import('../types').GoalParamsType} */
    let params;
  ...
  $: parsedGoal = parseRawGoalData(goal);
  $: isByAmount = parsedGoal.type === "byAmount";
  $: isByPercent = parsedGoal.type === "byPercent";
</script>
...
{#if parsedGoal}
  <article class="panel">
    ...
    <div class="panel-content">
      {#if isByAmount}
        <div>
          Collect <span class="emphasis">
            {parsedGoal.params.eventsAmount}
          </span>
          FLOATs
          {#if parsedGoal.params.requiredEventsAmount > 0}
            with
            <Badge required />
            <span class="emphasis">
              {parsedGoal.params.requiredEventsAmount}
            </span>
          {/if}
        </div>
      {:else if isByPercent}
        <div>
          Collect <span class="emphasis">{parsedGoal.params.percent}% </span>
          FLOATs of the series
        </div>
      {:else}
        <div class="flex-wrap flex-gap">
          {#each parsedGoal.params.events as one (`${one.host}#${one.id}`)}
            <EventItem
      ...
      {/if}
      <span class="emphasis">{parsedGoal.points} Point</span>
    </div>
  </article>
{/if}
