        ...
        {#each rows2 as row, index (row)}
          <Row {index} on:click={() => onCellClick(row)}>
            ...
            <td data-label="Address"
              ><span class="mono"
                ><a href="/{row.address}">{row.address}</a></span
              ></td>
          </Row>
        {/each}
      </tbody>
...
{:else}
  <p>
    This FLOAT is very popular! 🎉 To be mindful of network load, we won't load
    the list of who claimed this FLOAT automatically. You are welcome to click
    on the button below to load it manually!
  </p>
  <button on:click={() => (showClaims = true)}
    >View list of who claimed this FLOAT</button>
{/if}
